...
class Tencent_api implements api_login {
	...
	public function callback()
	{
		...
		if (es_session::get("t_access_token")|| (es_session::get("t_openid")&&es_session::get("t_openkey"))) 
		{
			...
			{
				//当有用户身份且要求绑定时
				$GLOBALS['db']->query("update ".DB_PREFIX."user set tencent_id= '".$name."', t_access_token ='".$access_token."',t_openkey = '".$openkey."',t_openid = '".$openid."' where id =".$GLOBALS['user_info']['id']);						
				app_redirect(url("shop","uc_center#setweibo"));
			}
			else
