...
class cate_list{
	...
	function getCateArray($cate_type,$pid){
		...
		if ($cate_type == 2){
			...
			$sql = "select id,name,pid,uname as py, '' as icon from ".$table_name. " where is_effect =1 and pid = ".$pid;
			$list = $GLOBALS['db']->getAll($sql);
			foreach($list as $k=>$v)
			{
				$count = intval($GLOBALS['db']->getOne("select count(*) from ".$table_name." where is_effect =1 and pid = ".$v['id']));
				if($count>0)
				$list[$k]['has_child'] = 1;
				else
				$list[$k]['has_child'] = 0;
