...
class merchantlist
{
	public function index()
	{
		...
		if ($quan_id > 0){
			$sql_q = "select name from ".DB_PREFIX."area where id = ".intval($city_id);
		...
		if($keyword){
			...
			//有筛选
			$where .=" and (match(a.name_match,a.locate_match,a.deal_cate_match,a.tags_match) against('".$kw_unicode."' IN BOOLEAN MODE))";	
	   }
	   $where.=$condition;
