 ...
 * 获取商家店面列表
 */
function get_store_list($limit, $cate_id=0, $where='',$orderby = '',$cached = true)
{		
		...
		if($res===false)
		{
	 		...
	 		if($stores)
			{
				...
				{					
					...
					$stores[$k] = $store;
					$stores[$k]['group_point'] = $GLOBALS['db']->getAll("select id,name from ".DB_PREFIX."point_group as pg left join ".DB_PREFIX."point_group_link as pgl on pg.id = pgl.point_group_id  where pgl.category_id = ".$store['deal_cate_id']." order by sort asc" );
					foreach($stores[$k]['group_point'] as $kk=>$vv)
					{
						$stores[$k]['group_point'][$kk]['avg_point'] =  round(floatval($GLOBALS['db']->getOne("select avg_point from ".DB_PREFIX."supplier_location_point_result where supplier_location_id = ".$store['id']." and group_id = ".$vv['id'])),1);
					}
				}
			}	
