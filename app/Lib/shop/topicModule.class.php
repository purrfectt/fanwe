...
class topicModule extends ShopBaseModule
{
	public function index()
	{
			...
			$group_id = intval($message_item['group_id']);
			...
			$message_item['group_name'] = $GLOBALS['db']->getOne("select name from ".DB_PREFIX."topic_group where id = ".$message_item['group_id']);
	...
	{		
		...
		{			
			...
			if($v['city_id']!=0&&$v['city_id']==$city_id)
			{
...
//		$result = get_topic_list($limit,$condition,"",$kws_div);
		...
		//输出搜索热词
		$hot_tag_list =$GLOBALS['db']->getAll("select name,color from ".DB_PREFIX."topic_tag where is_recommend = 1 order by sort desc, count desc limit 10");
		$hot_title_list =$GLOBALS['db']->getAll("select name,color from ".DB_PREFIX."topic_title where is_recommend = 1 order by sort desc,count desc limit 10");
		$GLOBALS['tmpl']->assign("hot_tag_list",$hot_tag_list);
		$GLOBALS['tmpl']->assign("hot_title_list",$hot_title_list);
