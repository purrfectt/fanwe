...
{		
		$time = get_gmtime();
		if($id>0&&$preview==0)
		$deal = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal where is_shop = 0 and id = ".intval($id)." and is_effect = 1 and is_delete = 0 and (".$time.">= begin_time or begin_time = 0 or notice = 1) ");
		if($id>0&&$preview>0)
		$deal = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal where is_shop = 0 and id = ".intval($id)." and is_delete = 0");
...
{		
		...
		{
			$city_name = $GLOBALS['db']->getOne("select name from ".DB_PREFIX."deal_city where id = ".$city_id);
		}
